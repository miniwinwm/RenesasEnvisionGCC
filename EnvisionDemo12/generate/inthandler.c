...
extern volatile uint32_t sections;
extern volatile uint32_t section_offset;
extern volatile uint32_t section_transfers;
...
{
	/* check for all sections completed */
	if (sections == 0U)
	{
		dma_complete = true;
		return;
	...
	/* flip section start request mode to software */
	DMAC0.DMTMD.BIT.DCTG = 0U;

	/* update destination address moving over offset */
	DMAC0.DMDAR += (section_offset - (section_transfers * sizeof(uint16_t)));

	/* reset number of transfers in this section */
	DMAC0.DMCRA = section_transfers;

	/* request software transfer */
